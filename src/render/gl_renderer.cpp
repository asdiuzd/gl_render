...
Renderer::Renderer(const std::string shader_path, const std::string shader_name, const std::string obj_path)
{
    ...
    shaderGeometryPass = Shader((shader_path + "/" + shader_name + ".vs").c_str(),
                                (shader_path + "/" + shader_name + ".fs").c_str());
...
float *Renderer::render_single_frame(Eigen::Quaternionf rotation, Eigen::Vector3f translation, Eigen::Matrix3f intrinsics,
                                     string filename, string output_path, bool return_buffer)
{
